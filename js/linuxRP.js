async function generarReleasePlan_linux(data) {
    ...
    try {
                    ...
                    new TableRow({
                        children: row.slice(0, 8).map((cellContent, idx) => {
                            ...
                            if (typeof cellContent === 'string') {
                                ...
                                // Y dividir por \n si lo contiene
                                contentArray = [];
                                cellContent.forEach((it) => {
                                    if (typeof it.text === 'string' && it.text.includes('\n')) {
                                        // Dividir por \n y crear múltiples items
                                        const lines = it.text.split('\n');
                                        lines.forEach((line) => {
                                            contentArray.push({
                                                ...it,
                                                text: insertSoftWraps(line)
                                            });
                                        });
                                    } else {
                                        contentArray.push({
                                ...
                                // Si el texto contiene saltos de línea, dividirlo
                                if (typeof it.text === 'string' && it.text.includes('\n')) {
                                    contentArray = it.text.split('\n').map(line => ({
                                        ...it,
                                        text: insertSoftWraps(line)
                                    }));
                                } else {
        ...
        function buildSections(info) {
            ...
            // Helper: Detectar sistema basado en el campo sistemas o prefijo de solicitud
            const getSystemName = (solicitud, sistemas) => {
                // Priorizar el campo 'sistemas' si está disponible
                if (sistemas) {
                    const syst = String(sistemas).toUpperCase().trim();
                    console.log('DEBUG getSystemName - sistemas:', sistemas, 'syst:', syst);
                    
                    if (syst.includes('TCO')) {
                        console.log('Detectado TCO en sistemas -> Tuca');
                        return 'Tuca';
                    }
                    if (syst.includes('TAM')) {
                        console.log('Detectado TAM en sistemas -> Tamsa');
                        return 'Tamsa';
                    }
                }
                
                // Fallback: intentar con el prefijo de solicitud
                if (solicitud) {
                    const fullSolicitud = String(solicitud).toUpperCase().trim();
                    console.log('DEBUG getSystemName - solicitud:', solicitud, 'fullSolicitud:', fullSolicitud);
                    
                    if (fullSolicitud.startsWith('TCO')) {
                        console.log('Detectado TCO en solicitud -> Tuca');
                        return 'Tuca';
                    }
                    if (fullSolicitud.startsWith('TAM')) {
                        console.log('Detectado TAM en solicitud -> Tamsa');
                        return 'Tamsa';
                    }
                }
                
                console.log('No detectado prefijo conocido -> Tamsa (default)');
                return 'Tamsa'; // Default fallback
            };

            // Helper: Crear texto de solicitud formateado
            const formatSolicitud = (solicitud, nombre, sistemas) => {
                const system = getSystemName(solicitud, sistemas);
                return system + ': ' + (solicitud || '') + ' - ' + (nombre || '');
            };

            // Recopilar todas las solicitudes (incluida la principal y las adicionales)
            const allSolicitudes = [];
            
            // Agregar la solicitud principal
            if (data.solicitud || data.nombre_solicitud) {
                allSolicitudes.push({
            ...
            // Agregar las solicitudes adicionales de Linux
            try {
                const linuxExtras = Array.isArray(data.extra_cases) ? data.extra_cases.filter(c => c && c.type === 'linux') : [];
                linuxExtras.forEach(c => {
                    allSolicitudes.push({
                        solicitud: c.solicitud,
                        nombre_solicitud: c.nombre_solicitud,
                        sistemas: c.sistemas
                    });
                });
            } catch(_) {}

            // Construir filas según si hay múltiples solicitudes o no
            const rowsRP6 = [exampleHeaders];
            let seqNumber = 1;

            if (allSolicitudes.length > 1) {
                // Caso: múltiples solicitudes
                // Fila 1: Autorización de TODAS las solicitudes
                const allSolicitudesText = allSolicitudes.map((s, i) => 
                    formatSolicitud(s.solicitud, s.nombre_solicitud, s.sistemas)
                ).join('\n');

                const authAllRow = [[
                    { text: 'Autorizar todas las solicitudes' },
                    { text: ' ' },
                    { text: allSolicitudesText, bold: true },
                    { text: ' ' },
                ...
                rowsRP6.push(authAllRow);

                // Filas de ejecución: una por cada solicitud
                allSolicitudes.forEach(s => {
                    const execRow = [[
                        { text: 'Ejecución de la solicitud ' },
                        { text: formatSolicitud(s.solicitud, s.nombre_solicitud, s.sistemas), bold: true }
                    ], String(seqNumber++), [{ text: data.usuario.nombre }, { text: data.usuario.email }], 'ASAP', ' ', ' ', ' ', { text: '[Pending]', highlight: 'yellow', underline: UnderlineType.SINGLE }];

                    rowsRP6.push(execRow);
                });
            } else {
                // Caso: una solicitud única (comportamiento original)
                const exampleRow = [[{ text: 'Autorizar la solicitud' },
                { text: ' ' },
                { text: formatSolicitud(data.solicitud, data.nombre_solicitud, data.sistemas), bold: true },
                { text: ' ' },
                { text: 'desde el comando APT - Soporte' }
                ], '1',
                [{ text: 'MARTINEZ R. Benjamin F.' },
                ...
                { text: formatSolicitud(data.solicitud, data.nombre_solicitud, data.sistemas), bold: true }
                ], '2', [{ text: data.usuario.nombre },
                { text: data.usuario.email }
                ], 'ASAP', ' ', ' ', ' ', { text: '[Pending]', highlight: 'yellow', underline: UnderlineType.SINGLE }];

                rowsRP6.push(exampleRow);
                rowsRP6.push(exampleRow2);
            }
